...
def load_config():
    ...
    if os.path.exists(config_file):
        try:
            with open(config_file, 'r') as f:
                ...
                heartbeat_interval = float(config.get('HEARTBEAT_INTERVAL_SECONDS', 10))

                if token is not None:
                    return ws_url, token, heartbeat_interval
    ...
    heartbeat_interval = float(os.environ.get("HEARTBEAT_INTERVAL_SECONDS", 10))
...
async def test_ws_connection():
    ...
    # Load configuration
    ws_url, token, heartbeat_interval = load_config()
...
if __name__ == "__main__":
    ...
    if not token:
        ...
        print('  "HEARTBEAT_INTERVAL_SECONDS": "10"')
        print("}")
        print("\nOr set the environment variables: BACKEND_WS_URL, BMAD_AUTH_TOKEN, HEARTBEAT_INTERVAL_SECONDS.")
        print("\nTo get a token, run:")
