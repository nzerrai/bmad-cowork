...
- **Language** — Speak in `french`. Write any file output in `french`.
...
3. Otherwise — scan artifacts and ask
   - Active specs (`draft`, `ready-for-dev`, `in-progress`, `in-review`) in `/Users/nouredinezerrai/projets/bmad-portal0/prjdocs/implementation-artifacts`? → List them and HALT. Ask user which to resume (or `[N]` for new).
...
1. Load context.
   ...
   - **Determine context strategy.** Using the intent and the artifact listing, infer whether the current work is a story from an epic. Do not rely on filename patterns or regex — reason about the intent, the listing, and any epics file content together.
     ...
     **A) Epic story path** — if the intent is clearly an epic story:
     ...
     3. **Compile epic context.** Produce `/Users/nouredinezerrai/projets/bmad-portal0/prjdocs/implementation-artifacts/epic-<N>-context.md` by following `./compile-epic-context.md`, in order of preference:
        - **Preferred — subagent:** spawn a subagent synchronously (wait for it to return in this turn) with `./compile-epic-context.md` as its prompt. Pass it the epic number, the epics file path, the `/Users/nouredinezerrai/projets/bmad-portal0/prjdocs/planning-artifacts` directory, and the output path `/Users/nouredinezerrai/projets/bmad-portal0/prjdocs/implementation-artifacts/epic-<N>-context.md`.
...
4. Multi-goal check (see SCOPE STANDARD). If the intent fails the single-goal criteria:
   ...
   - On **S**: For each deferred goal, append one new entry to `/Users/nouredinezerrai/projets/bmad-portal0/prjdocs/implementation-artifacts/deferred-work.md` using this format. Do not modify existing entries or look for duplicates. Narrow scope to the first-mentioned goal. Continue routing.
     ```markdown
     - source_spec: none
