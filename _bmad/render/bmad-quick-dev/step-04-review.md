...
If a layer's instruction requires subagents and none are available, generate one review prompt file per such layer in `/Users/nouredinezerrai/projets/bmad-portal0/prjdocs/implementation-artifacts` and HALT. Ask the human to run each in a separate session (ideally a different LLM) and paste back the findings.
